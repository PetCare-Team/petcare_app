...
class CreatePet : Fragment() {
    ...
    override fun onCreateView(
        ...
        val imReturn = view.findViewById<ImageView>(R.id.imReturn)
...
//        val id=arguments?.getInt(ARG_PERRO_ID, -1)!!
        ...
        val call = apiService.CreatePet(petUpdate)
        call.enqueue(object : Callback<SavePetResource> {
            override fun onResponse(call: Call<SavePetResource>, response: Response<SavePetResource>) {
                if (response.isSuccessful) {
                    ...
                    Toast.makeText(requireContext(), "Perfil creado exitosamente", Toast.LENGTH_LONG).show()
                } else {
                    // Hubo un error en la actualización
                    Toast.makeText(requireContext(), "Error en la creacion: ${response.errorBody()?.string()}", Toast.LENGTH_LONG).show()
                }
            }
